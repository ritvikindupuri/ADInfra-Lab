...
* [cite_start]**Boot Configuration:** Systems were configured to use **UEFI Boot** with Secure Boot disabled to allow the Ventoy USB to load the Windows Server 2022 ISO[cite: 38, 40].
* [cite_start]**Disk Partitioning:** Critical care was taken to install the OS specifically on the **NVMe (~250GB)** drive, strictly avoiding the 1TB SATA drives to ensure performance and separation of concerns[cite: 53].
* [cite_start]**Driver Injection:** Post-installation, Intel PCIe Ethernet drivers were manually injected via Device Manager ("Have Disk" method) to enable network connectivity[cite: 66, 81].
...
**Dashboard Forensics:**
* [cite_start]**Service Health:** The green indicators on **AD DS** (Active Directory Domain Services) and **DNS** confirm that the core identity services are running and healthy[cite: 83].
* **Pending Actions:** The red status on the **All Servers** tile typically indicates a post-promotion warning, such as a pending restart or a service (like "Download Maps Manager") that is set to delayed start. [cite_start]In a production environment, this would trigger a maintenance window for remediation[cite: 27].
...
* **DNS Forwarding:**
    * [cite_start]**Preferred DNS:** Set to `127.0.0.1` (loopback) to ensure the DC queries its own local records first[cite: 85].
    * **Alternate DNS:** Pointed to the upstream CIT DNS server (`cit.lcl`) to facilitate external name resolution for updates.
    * [cite_start]**Time Sync:** NTP settings were synchronized with the `cit.lcl` time servers using PowerShell to prevent Kerberos ticket failures due to time drift[cite: 33].
...
* [x] **Network:** Static IPs assigned; [cite_start]IPv6 disabled[cite: 19].
* [x] **Active Directory:**
...
* [cite_start]**Driver Issues:** If the Ethernet controller does not appear, the specific Intel drivers (Version 12.19.2.45) were manually located on the USB drive[cite: 74].
* [cite_start]**Promotion Failures:** If a child domain fails to join the forest, verify that its **Preferred DNS** is temporarily set to the IP address of the **Forest Root Server** (Server 01) to allow it to locate the schema master[cite: 62].
